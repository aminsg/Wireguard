"""
< Wireguard Panel > - Copyright(C) 2021 Donald Zou [https://github.com/Azumi67]
...
def get_latest_handshake(config_name):
    ...
    now = datetime.now()
    time_delta = timedelta(minutes=2)
    peers = g.cur.execute(f"SELECT id, ends_at, is_connected, created_at FROM {config_name}").fetchall()

    for peer in peers:
        (key, ends_at, is_connected, created_at) = peer
        ...
        if handshake:
            ...
            if status == "running" and not bool(is_connected):
                ends_at = int(ends_at) + time.time() - int(created_at) if ends_at else ends_at
                is_connected = 1

            if handshake['time'] > 0:
                query = f"update {config_name} set latest_handshake = ?, status = ?, is_connected = ?, ends_at = ? where id=?"
                bindings = (str(diff).split('.', maxsplit=1)[0], status, is_connected, ends_at, key)
...
def get_transfer(config_name):
    ...
    peers = g.cur.execute(
        "SELECT total_receive, total_sent, cumu_receive, cumu_sent, status, bandwidth, is_enabled, ends_at, id FROM %s" % (
            config_name)).fetchall()

    if len(peers) > 0:
        for peer in peers:
            (total_receive, total_sent, cumu_receive, cumu_sent, status, bandwidth, is_enabled, ends_at, key) = peer
            ...
            if status == "running":
                ...
                if is_enabled:
                    is_enabled = ((ends_at is None or time.time() < int(ends_at)) and (
                            bandwidth == 0 or bandwidth >= total_sent * pow(1024, 3)))
        ...
        if "PublicKey" in conf_peer_data['Peers'][i].keys():
            ...
            if len(result) == 0:
                ...
                INSERT INTO {config_name} 
                    ...
                    :cumu_data, :mtu, :keepalive, :remote_endpoint, :preshared_key, :is_enabled, :ends_at, :bandwidth, :is_connected, :created_at);
                """
...
def get_peers(config_name, search="", sort_t="status"):
    ...
    def cast_data(item):
        item.update({
            "ends_at": datetime.fromtimestamp(item['ends_at']).isoformat() if item.get('ends_at') else None,
...
def add_peer(config_name):
    ...
    bandwidth = float(data['bandwidth']) * pow(1024, 3) if data.get('bandwidth') else 0
    ...
    try:
        ...
        status = subprocess.check_output("wg-quick save " + config_name, shell=True, stderr=subprocess.STDOUT)
        get_all_peers_data(config_name)
        sql = "UPDATE " + config_name + " SET name = ?, private_key = ?, DNS = ?, endpoint_allowed_ip = ?, bandwidth = ?, ends_at = ?, is_connected = ?, created_at = ? WHERE id = ?"
        g.cur.execute(sql, (
...
def save_peer_setting(config_name):
    ...
    if peer:
        ...
        is_enabled = (ends_at is None or int(ends_at) > now) and (
        ...
        try:
            ...
            sql = "UPDATE " + config_name + " SET name = ?, bandwidth = ?, private_key = ?, DNS = ?, endpoint_allowed_ip = ?, mtu = ?, keepalive = ?, preshared_key = ?, is_enabled = ?, ends_at = ? WHERE id = ?"
...
def get_peer_name(config_name):
    ...
    peer_id = data['id']
    result = g.cur.execute(
        "SELECT name, allowed_ip, DNS, private_key, endpoint_allowed_ip, mtu, keepalive, preshared_key, bandwidth, ends_at, is_enabled FROM "
        + config_name + " WHERE id = ?", (peer_id,)).fetchall()
    data = {"name": result[0][0], "bandwidth": result[0][8], "is_enabled": bool(result[0][10]), "allowed_ip": result[0][1],
            "DNS": result[0][2],
