...
def get_latest_handshake(config_name):
    ...
    now = datetime.now()
    time_delta = timedelta(minutes=2)
    peers = g.cur.execute(f"SELECT id, ends_at, timer_on, created_at FROM {config_name}").fetchall()

    for peer in peers:
        (key, ends_at, timer_on, created_at) = peer
        timer_on = timer_on or 0
        ...
        if handshake:
            ...
            if status == "running" and not bool(timer_on):
                ends_at = int(ends_at) + time.time() - int(created_at) if ends_at else ends_at
                timer_on = 1

            if handshake['time'] > 0:
                query = f"update {config_name} set latest_handshake = ?, status = ?, timer_on = ?, ends_at = ? where id=?"
                bindings = (str(diff).split('.', maxsplit=1)[0], status, timer_on, ends_at, key)
...
def get_transfer(config_name):
    ...
    peers = g.cur.execute(
        "SELECT total_receive, total_sent, cumu_receive, cumu_sent, status, bandwidth, end_active, ends_at, id FROM %s" % (
    ...
    if len(peers) > 0:
        for peer in peers:
            ...
            if status == "running":
                ...
                if end_active:
                    end_active = ((ends_at is None or time.time() < int(ends_at)) and (
                ...
                g.cur.execute(
                    "UPDATE %s SET total_receive = %f, total_sent = %f, total_data = %f, end_active=%d, status='%s' WHERE id = '%s'" %
                    (config_name, round(total_receive, 4), round(total_sent, 4),
                     round(total_receive + total_sent, 4), int(end_active), status, key))
...
def get_peers(config_name, search="", sort_t="status"):
    ...
    def cast_data(item):
        item.update({
            ...
            "end_active": bool(item['end_active']) if item.get('end_active') else False
        })
