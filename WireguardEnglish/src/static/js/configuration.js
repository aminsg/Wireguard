...
(function () {
    ...
    function configurationHeader(response) {
        ...
        if (response.checked === "checked") {
            $conf_status_btn.innerHTML = `<a href="#" id="${response.name}" ${response.checked} class="switch text-primary"><i class="bi bi-toggle2-on"></i> ONLINE</a>`;
        } else {
            $conf_status_btn.innerHTML = `<a href="#" id="${response.name}" ${response.checked} class="switch text-primary"><i class="bi bi-toggle2-off"></i> OFFLINE</a>`;
        }
    ...
    function configurationPeers(response) {
        ...
        if (response.peer_data.length === 0) {
            document.querySelector(".peer_list").innerHTML = `<div class="col-12" style="text-align: center; margin-top: 1.5rem"><h3 class="text-muted"> You have no peers at the moment （●´∧｀●）</h3></div>`;
        } else {
            ...
            response.peer_data.forEach(function (peer) {
                ...
                let peer_name =
                    ...
                    '<h6 style="text-transform: uppercase; margin: 0; margin-left: auto !important;"><span class="dot dot-' + peer.status + '" style="margin-left: auto !important;" data-toggle="tooltip" data-placement="left" title="Connected Peers"></span></h6>' +
                    '</div>';
                ...
                let peer_key = '<div class="col-sm"><small class="text-muted" style="display: flex"><strong>User</strong><strong style="margin-left: auto!important; opacity: 0; transition: 0.2s ease-in-out" class="text-primary">Click to Copy</strong></small> <h6><samp class="ml-auto key">' + peer.id + '</samp></h6></div>';
                let peer_allowed_ip = '<div class="col-sm"><small class="text-muted"><strong>Wireguard IP</strong></small><h6 style="text-transform: uppercase;">' + peer.allowed_ip + '</h6></div>';
                ...
                let peer_endpoint = '<div class="col-sm"><small class="text-muted"><strong>User IP</strong></small><h6 style="text-transform: uppercase;">' + peer.endpoint + '</h6></div>';
                let peer_enable = '<div class="col-sm"><small class="text-muted"><strong>Status</strong></small><h6 style="text-transform: uppercase;">' + (peer.end_active ? 'Enabled' : 'Disabled') + '</h6></div>';
                let peer_control = '<div class="col-sm"><hr><div class="button-group" style="display:flex"><button type="button" class="btn btn-outline-primary btn-setting-peer btn-control" id="' + peer.id + '" data-toggle="modal"><i class="bi bi-gear-fill" data-toggle="tooltip" data-placement="bottom" title="User Setting"></i></button> <button type="button" class="btn btn-outline-danger btn-delete-peer btn-control" id="' + peer.id + '" data-toggle="modal"><i class="bi bi-x-circle-fill" data-toggle="tooltip" data-placement="bottom" title="Delete User"></i></button>';
                if (peer.private_key !== "") {
    ...
    function addPeersByBulk() {
        ...
        if ($new_add_amount.val() > 0 && !$new_add_amount.hasClass("is-invalid")) {
            if ($new_add_DNS.val() !== "" && $new_add_endpoint_allowed_ip.val() !== "") {
                ...
                $("#add_peer_alert").html("Fill all the required information").removeClass("d-none");
                $add_peer.removeAttribute("disabled");
    ...
    function loadPeers(searchString) {
        ...
        $.ajax({
            ...
            $(".dot.dot-running").attr("title", "User is connected").tooltip();
            $(".dot.dot-stopped").attr("title", "User has been disconnected").tooltip();
...
$body.on("click", ".btn-setting-peer", function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $("#setting_modal #peer_end_textbox").attr("data-end-time", response.ends_at);
            const endTime = $("#setting_modal #peer_end_textbox").attr("data-end-time");
            $("#setting_modal #peer_end_textbox").val(formatDate(new Date(endTime)));  
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.peers.length > 0) {
                ...
                window.configurations.showToast("Users Zipfiles successfully downloaded!");
            } else {
                window.configurations.showToast("You have no users at the moment!");
            }
        }
    });
});