    ...
    function configurationPeers(response) {
        ...
        if (response.peer_data.length === 0) {
            ...
            response.peer_data.forEach(function (peer) {
                ...
                let peer_key = '<div class="col-sm"><small class="text-muted" style="display: flex"><strong>کاربر</strong><strong style="margin-left: auto!important; opacity: 0; transition: 0.2s ease-in-out" class="text-primary">کلیک تا کپی شود</strong></small> <h6><samp class="ml-auto key">' + peer.id + '</samp></h6></div>';
                let peer_allowed_ip = '<div class="col-sm"><small class="text-muted"><strong>ایپی وایرگارد</strong></small><h6 style="text-transform: uppercase;">' + peer.allowed_ip + '</h6></div>';
                let peer_latest_handshake = '<div class="col-sm"> <small class="text-muted"><strong>اخرین ارتباط</strong></small> <h6 style="text-transform: uppercase;">' + peer.latest_handshake + '</h6> </div>';
                let peer_endpoint = '<div class="col-sm"><small class="text-muted"><strong>ایپی کاربر</strong></small><h6 style="text-transform: uppercase;">' + peer.endpoint + '</h6></div>';
                let peer_enable = '<div class="col-sm"><small class="text-muted"><strong>وضعیت</strong></small><h6 style="text-transform: uppercase;">' + (peer.is_enabled ? 'فعال' : 'غیرفعال') + '</h6></div>';
    ...
    function addPeersByBulk() {
        ...
        if ($new_add_amount.val() > 0 && !$new_add_amount.hasClass("is-invalid")) {
            if ($new_add_DNS.val() !== "" && $new_add_endpoint_allowed_ip.val() !== "") {
                ...
                $("#add_peer_alert").html("تمام اطلاعات مورد نیاز را پرکنید").removeClass("d-none");
                $add_peer.removeAttribute("disabled");
