...
# wgd.sh - Copyright(C) 2021 Donald Zou [https://github.com/Azumi67]
...
help () {
  printf "=================================================================================\n"
  printf "+          <Wireguard Panel> by Donald Zou - https://github.com/Azumi67           +\n"
  printf "=================================================================================\n"
  printf "| Usage: ./wgd.sh <option>                                                      |\n"
  printf "|                                                                               |\n"
  printf "| Available options:                                                            |\n"
  printf "|    start: To start Wireguard Panel.                                               |\n"
  printf "|    stop: To stop Wireguard Panel.                                                 |\n"
  printf "|    debug: To start Wireguard Panel in debug mode (i.e run in foreground).         |\n"
  printf "|    update: To update Wireguard Panel to the newest version from GitHub.           |\n"
  printf "|    install: To install Wireguard Panel                                           |\n"
...
install_wgd(){
    ...
    printf "| Wireguard Panel installed successfully!                      |\n"
...
gunicorn_start () {
  printf "%s\n" "$dashes"
  printf "| Starting Wireguard Panel with Gunicorn in the background.    |\n"
  if [ ! -d "log" ]; then
    mkdir "log"
  fi
  d=$(date '+%Y%m%d%H%M%S')
...
start_wgd_debug() {
  printf "%s\n" "$dashes"
  printf "| Starting Wireguard Panel in the foreground.                  |\n"
  python3 "$app_name"
  printf "%s\n" "$dashes"
}
...
if [ "$#" != 1 ];
  then
    ...
    if [ "$1" = "start" ]; then
        if check_wgd_status; then
          printf "%s\n" "$dashes"
          printf "| Wireguard Panel is already running.                          |\n"
          printf "%s\n" "$dashes"
          else
            start_wgd
        fi
        ...
        if check_wgd_status; then
            printf "%s\n" "$dashes"
            stop_wgd
            printf "| Wireguard Panel is stopped.                                  |\n"
            printf "%s\n" "$dashes"
            else
              printf "%s\n" "$dashes"
              printf "| Wireguard Panel is not running.                              |\n"
              printf "%s\n" "$dashes"
        fi
      elif [ "$1" = "update" ]; then
        update_wgd
